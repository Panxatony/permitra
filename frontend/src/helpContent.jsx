...
/* The help text itself, kept apart from the page that lists it so the same
   section can also open in place - the "?" beside a feature explains it over
   the page you are standing on, instead of navigating you to the help page and
   leaving you there when the overlay closes.

   Content is one bilingual structure rather than i18n entries: these are pages
   of prose keyed by topic, not labels keyed by English text, and the instance
   language decides which half renders. */
  ...
  },
  {
    ...
    de: {
      ...
      body: [
        ...
        { ol: [
          'Ein **Change Approver startet** sie: Name, Stichtag und Umfang — `all` (alle Regeln in Kraft), `zone:Z040` (alle Regeln aus oder in die Zone) oder `component:3` (alle Regeln dieser Komponente). Die Liste wird beim Start eingefroren; später angelegte Regeln gehören zur nächsten Kampagne.',
    ...
    en: {
      ...
      body: [
        ...
        'How a campaign runs:',
        { ol: [
          'A **change approver starts** it: name, cut-off date and scope — `all` (every rule in force), `zone:Z040` (every rule out of or into the zone) or `component:3` (every rule on that component). The list is frozen at start; rules created later belong to the next campaign.',
...
export function teaser(section) {
...
/* The body of one section, rendered the same way wherever it is shown. */
export function HelpBody({ section }) {
  const { lang } = useLang()
...
export function helpTitle(section, lang) {
  return (lang === 'de' ? section.de : section.en).title
}
