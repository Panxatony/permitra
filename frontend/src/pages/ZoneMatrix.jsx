...
/* Module scope, so `t` and `lang` have to be handed in rather than taken from
   the context - the export helpers below run outside the React tree. */
function planTitle(t, lang) {
  return `${t('Zone plan (cleaned-up network diagram)')} · BSI NET.1.1 / NET.3.2 · `
    + `${t('as of')} ${new Date().toLocaleString(dateLocale(lang))} · ${t('generated by Permitra')}`
}

function exportPlanPng(t, lang) {
  ...
  img.onload = () => {
    ...
    ctx.fillText(planTitle(t, lang), 20, 38)
...
function exportPlanPdf(t, lang) {
  const plan = inlinedPlanSvg()
  if (!plan) return
  const win = window.open('', '_blank')
  win.document.write(`<!DOCTYPE html><html lang="${lang}"><head><title>${t('Permitra zone plan')}</title>
    ...
    <body><h1>${planTitle(t, lang)}</h1>${plan.markup}
  ...
  return (
    <div className={`topology-wrap zone-reach${hover ? ' has-hover' : ''}`}>
      <div className="plan-head">
        <div>
          <strong>{t('Zone plan (cleaned-up network diagram)')}</strong>
          <span className="muted small"> · BSI NET.1.1 / NET.3.2 · {t('as of')}{' '}
            {new Date().toLocaleString(dateLocale(lang))} · {t('generated by Permitra')}</span>
        </div>
        <div className="plan-actions">
          <button type="button" className="btn btn-ghost" onClick={() => exportPlanPng(t, lang)}>PNG</button>
          <button type="button" className="btn btn-ghost" onClick={() => exportPlanPdf(t, lang)}>PDF</button>
          ...
          return zones.map((z) => z.firewalls.map((f) => {
            ...
            return (
              <path key={`${z.name}-${f.id}`}
                ...
                <title>{t('{zone} is reachable via {firewall}').replace('{zone}', z.name).replace('{firewall}', f.name)}</title>
              </path>
            )
          }))
        })()}
...
export default function ZoneMatrix() {
  ...
  const itemLabel = (c) => {
    ...
    if (c.change_type === 'net_delete') {
      return t('Remove network {cidr} from zone {zone}')
        .replace('{cidr}', c.to_zone).replace('{zone}', c.from_zone)
    }
  ...
  const addZone = (e) => {
    ...
    if (zones.some((z) => z.name.toUpperCase() === name.toUpperCase() || (z.code || '').toUpperCase() === code.toUpperCase())
      ...
      setError(t("Zone '{code} / {name}' already exists.").replace('{code}', code).replace('{name}', name))
      return
    }
    setError('')
          ...
          <div className="zone-chips">
            {zones.map((z) => (
              <span key={z.id} className="zone-chip">
                {z.name}
                <button type="button" title={t('Delete zone {name}').replace('{name}', z.name)}
